...
fn process_dns_spoof(
    msg: &mut nfq::Message,
    redirect_to_ip: String,
    domain_name: String,
) -> Verdict {
    ...
    let from_python = Python::with_gil(|py| -> Result<Vec<u8>, PyErr> {
        ...
        let args = PyTuple::new(
            py,
            &[
                domain_name.to_object(py),
                redirect_to_ip.to_object(py),
                data.to_object(py),
            ],
        );
...
pub fn run(dns_spoof: &DNSSpoof, running: &Arc<AtomicBool>) -> Result<(), std::io::Error> {
    ...
    loop {
            ...
            Err(e) => {
                match queue.unbind(QUEUE_NUMBER) {
                    ...
                    Err(e) => {
                        return Err(std::io::Error::new(
                            std::io::ErrorKind::Other,
                            format!("Error unbinding queue: {}", e),
                        ))
                    }
                };
        ...
        let verdict = process_dns_spoof(
            &mut msg,
            dns_spoof.redirect_to.clone(),
            dns_spoof.domain.clone(),
        );
